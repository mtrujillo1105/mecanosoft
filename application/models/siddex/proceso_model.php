...
class Proceso_model extends CI_Model {    
   ...
   public function listar($filter,$filter_not="",$number_items='',$offset=''){ 
        $where = array();
        $this->siddex->select('*');
        $this->siddex->from($this->table." as p",$number_items,$offset);
        ...
        if(isset($filter->numero) && $filter->numero!=''){
            if(is_array($filter->numero) && count($filter->numero)>0){
                $this->siddex->where_in("p.codigo",$filter->numero);
            }
            else{
                $this->siddex->where("p.codigo",$filter->numero);
            }
        } 
        $this->siddex->order_by('p.codigo');        
