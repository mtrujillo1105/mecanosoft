...
class Area_model extends CI_Model{
    ...
    public function seleccionar($default="",$value='',$tipot=""){
      ...
      public function seleccionar_c($default="",$value=''){
        $nombre_defecto = $default==""?":: Seleccione ::":$default;
        $arreglo = array($value=>$nombre_defecto);
        foreach($this->listar_c() as $indice=>$valor)
        {
            $indice1   = $valor->cod_argumento;
            $valor1    = $valor->des_larga;
            $arreglo[$indice1] = $valor1;
     ...
     public function listar_c($number_items='',$offset='')
    {
        $where = array('CodEnt'=>$this->entidad,'Cod_Tabla'=>'TCAR','Des_Corta'=>'02','Valor_1'=>1.0000);        
    ...
    public function listar($number_items='',$offset='',$tipot = '')
    {
//        echo "-----------HOLA------------";
//        exit;
        $where = array('CodEnt'=>$this->entidad,'Cod_Tabla'=>'APRO','VALOR_1'=>1);  
        if($tipot != "") {
            if($tipot == '04') {
                $this->db->where_not_in('cod_argumento','031');
            }
        } 
        $this->db->select('cod_tabla,cod_argumento,valor_2,des_larga,Des_Corta');
        $this->db->from($this->table,$number_items,$offset);
