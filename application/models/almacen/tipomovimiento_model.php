...
class Tipomovimiento_model extends CI_Model{
    ...
    public function listar($filter,$filter_not,$order_by='',$number_items='',$offset=''){
        ...
        if(isset($filter->tipomovimiento) && $filter->tipomovimiento!='')   $where = array_merge($where,array("p.TIPOMOVP_Codigo"=>$filter->tipomovimiento));
        $this->db->select('*');
    ...
    public function modificar($codigo,$data){
        $this->db->where("TIPOMOVP_Codigo",$codigo);
        $this->db->update($this->table,$data);
    }
	
    public function eliminar($codigo){
        $this->db->delete($this->table,array('TIPOMOVP_Codigo' => $codigo));        
    }
}
?>