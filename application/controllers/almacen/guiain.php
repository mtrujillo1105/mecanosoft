...
class Guiain extends CI_Controller {
    var $entidad;
    var $login;
    ...
    public function index(){
        redirect(almacen."ningreso/listar");
    }
    ...
    public function ver(){
        ...
        $ningreso_det    = $this->ningreso_model->listar_detalle($filter,$filter_not);
        $codot          = $ningreso_det[0]->codot;   
        $num            = $ningreso_det[0]->codres;   
        $ot             = $ningreso_det[0]->ot;
        $ruccli         = $ningreso_det[0]->ruccli;   
        $numero         = $ningreso_det[0]->numero;
        $serie           = $ningreso_det[0]->serie;
        $fecha          = $ningreso_det[0]->fecha;
        $numreq         = $ningreso_det[0]->numreq;
        $codres         = $ningreso_det[0]->codres;      
        $numcom         = $ningreso_det[0]->numcom;
        $sercom         = $ningreso_det[0]->sercom;
        $numoc          = $ningreso_det[0]->numoc;
        $moneda         = $ningreso_det[0]->moneda;
        $tcambio        = $ningreso_det[0]->tcambio;
        ...
        $filter2->ruccliente = $ruccli; 
        $clientes   = $this->proveedor_model->obtener($filter2,$filter2_not);
        $rsocial    = $clientes->RazCli;
        ...
        $filter2_not     = new stdClass();
        $filter2->codot  = $codot;
        $oOt             = $this->orden_model->obtener($filter2,$filter2_not);
        ...
        $CI->pdf->Cell(0,8, "NOTA DE INGRESO No ".$serie."-".$numero,0,1,"C",0);
        $CI->pdf->Cell(60,8, "",0,1,"L",0);
        $CI->pdf->SetFont('Arial','',7);
        $CI->pdf->Cell(120,5, "" ,0,1,"L",0);
        $CI->pdf->Cell(60,5, "FECHA                :  ".date_sql($fecha). '    '.$hora_Actual."                               No OC. : 001-".$numoc."               MONEDA: ".$moneda."             T.Cambio:".$tcambio,0,0,"L",0);
        $CI->pdf->Cell(60,5, "" ,0,0,"L",0);
        $CI->pdf->Cell(60,5,  "",0,1,"L",0);
        $CI->pdf->Cell(90,5, "PROVEEDOR :  ".$ruccli." - ".$rsocial,0,1,"L",0);
        ...
        $i = 1;
        foreach($ningreso_det as $indice=>$value){
            $codigo   = $value->codigo;
